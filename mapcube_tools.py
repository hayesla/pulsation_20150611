...
def mapcube_input(func):
    def check(*args, **kwargs):
        if not(isinstance(args[0], MapSequence)):
...
@mapcube_input
def running_difference(mc, offset=1, use_offset_for_meta='ahead', smoothy = False, smo = 1):
    """
    ...
    for i in range(0, len(mc.maps) - offset):
        if smoothy:
            new_data = smooth(mc[i + offset].data, smo) - smooth(mc[i].data,smo)
        else:
    ...
    return Map(new_mc, sequence=True)
...
@mapcube_input
def base_dif_relative(mc):
    # Create a list containing the data for the new map object
    base_data = mc[0].data
    min_val = np.min(base_data[np.nonzero(base_data)])
    base_data[base_data == 0 ] = min_val

    new_mc = []
    for m in mc:
        new_data = (m.data - base_data) / base_data
        new_mc.append(Map(new_data, m.meta))
...
@mapcube_input
def resample(mc, dimension, **kwargs):
    """
    Parameters
    ----------
    mc : sunpy.map.MapCube
    ...
    maps = []
    for m in mc:
        maps.append(m.resample(dimension, **kwargs))
    # Create the new mapcube and return
    return Map(maps, cube=True)
